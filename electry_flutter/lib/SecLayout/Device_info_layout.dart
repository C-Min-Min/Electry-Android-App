...
class DeviceInfoLayoutState extends State<DeviceInfoLayout> {
  ...
  void measurements(String selectDay) {
    ...
    setState(() {
      if (_measure != null) {
        for (var measureCount = 0;
            measureCount < _measure.length;
            measureCount++) {
          List<String> shorter =
              _measure[measureCount].timestamp.toString().split("T");
          String date;
          double hour;
          if ((int.parse(shorter[1].split(":")[0]) + 3) >= 24) {
            date = shorter[0].split("-")[0] +
                "-" +
                shorter[0].split("-")[1] +
                "-" +
                (int.parse(shorter[0].split("-")[2]) + 1).toString();
            hour = (int.parse(shorter[1].split(":")[0]) - 21) +
                (int.parse(shorter[1].split(":")[1]) / 60);
          } else {
            date = shorter[0];
            hour = (int.parse(shorter[1].split(":")[0]) + 3) +
                (int.parse(shorter[1].split(":")[1]) / 60);
          }
  ...
  Icon _icon(Device item) {
    Color clr;
    double size = Responsive.isDesktop(context) ? 100.0 : 60.0;
    if (item.devState == 1) {
      clr = Colors.black;
    } else {
      clr = colorPalette[0];
              ...
              children: <Widget>[
                ...
                new Padding(
                  child: new Text(
                    item.devDesc,
                    style: new TextStyle(
                        fontSize: Responsive.isDesktop(context) ? 28.0 : 18.0,
                        color: _color(item.devState, false),
  ...
  int lineCount;
  Future<void> _editStringDialog(
      BuildContext context, Device item, String change) {
    if (change == "name") {
      lineCount = 1;
      _textFieldController.value = _textFieldController.value.copyWith(
        text: item.devName,
        selection: TextSelection.collapsed(
          offset: _textFieldController.value.selection.baseOffset +
              item.devName.length,
        ),
      );
    } else if (change == "desc") {
